...
Producer::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  //before  chunksNumbersMap[dataNameDecoded] = 1; check if it as already a value assigned

  //chunksNumbersMap[dataNameDecoded] = 1;

      // Check if the key exists
    auto it = chunksNumbersMap.find(dataNameDecoded);

    if (it != chunksNumbersMap.end()) {
        // Key already exists, you can handle it accordingly
        std::cout << "Key " << dataNameDecoded << " already exists with value " << it->second << std::endl;
    } else {
        // Key doesn't exist, assign a new value
        chunksNumbersMap[dataNameDecoded] = 1;
        std::cout << "Key " << dataNameDecoded << " assigned with value 1" << std::endl;
    }

  if (chunksNumbersMap[dataNameDecoded] <= numberOfChunks) {
    ...
    std::string chunkName = filenameWithoutExtension + "#"
                            + std::to_string(chunksNumbersMap[dataNameDecoded]) + extension;
    ...
    m_transmittedDatas(data, this, m_face);
    m_appLink->onReceiveData(*data);
    chunksNumbersMap[dataNameDecoded]++;
