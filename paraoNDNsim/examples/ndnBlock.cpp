...
void cleanupNDNSimulation() {
    Simulator::Stop(Seconds(20.0));
    Simulator::Run();
    Simulator::Destroy();
}

void runNDNSimulation(const std::string& prefix) {
...
void handle_download(const std::string& filename, int start_chunk, int end_chunk) {
    ...
    for (int chunk_number = start_chunk; chunk_number <= end_chunk; ++chunk_number) {
        ...
        if (!chunk_filename.empty()) {
            ...
            std::string prefix = "/" + chunk_filename;
            runNDNSimulation(prefix);
            cleanupNDNSimulation();
