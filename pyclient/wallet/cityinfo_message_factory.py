...
# ------------------------------------------------------------------------------
'''
This file is used for automated testing of the cityinfo client.
'''

from sawtooth_processor_test.message_factory import MessageFactory

class cityinfoMessageFactory(object):
    def __init__(self, signer=None):
        self._factory = MessageFactory(
            family_name="cityinfo",
            family_version="1.0",
            namespace=MessageFactory.sha512("cityinfo".encode("utf-8"))[0:6],
            signer=signer)
    ...
    def _create_txn(self, txn_function, action, text=None):
        payload = ",".join([
            str(action), str(text)
        ]).encode()
    ...
    def create_tp_process_request(self, action, text=None):
        txn_function = self._factory.create_tp_process_request
        return self._create_txn(txn_function, action, text)

    def create_transaction(self, action, text=None):
    ...
    def create_set_request(self, text):
    ...
    def create_get_response(self, text):
        address = self._make_address()
        data = None
        if text is not None:
            data = str(text).encode()
